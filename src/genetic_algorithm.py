...
def _front_summary(front):
    ...
    return {
        ...
        "best_f1": min(ind.f[0] for ind in front),
        "best_f2": min(ind.f[1] for ind in front),
    }
...
def nsga2(
    ...
    if progress_callback:
        progress_callback(
            {
                "event": "start",
                "message": f"Pokretanje NSGA-II: N={N}, G={G}, pc={pc}, pm={pm}, seed={seed}",
                "generation": 0,
                "evaluated": 0,
                "total_expected": N * (G + 1),
            }
        )
    random.seed(seed)
    ...
    for index, individual in enumerate(population, start=1):
        while should_pause and should_pause():
            if progress_callback:
                progress_callback(
                    {
                        "event": "paused",
                        "message": "Algoritam je pauziran tokom inicijalne evaluacije.",
                        "generation": 0,
                        "evaluated": evaluated,
        ...
        if should_stop and should_stop():
            LOGGER.info("NSGA-II zaustavljen tokom inicijalne evaluacije.")
            break
        if progress_callback:
            cfg = decode(individual.x)
            progress_callback(
                {
                    "event": "candidate_start",
                    "message": f"Inicijalna evaluacija kandidata {index}/{N}",
                    "generation": 0,
                    "evaluated": evaluated,
                    "total_expected": total_expected,
                    "candidate_phase": "initial_population",
        ...
        if progress_callback:
            progress_callback(
                {
                    "event": "init_eval",
                    "message": f"Inicijalna evaluacija {index}/{N}",
                    "generation": 0,
                    "evaluated": evaluated,
                    "total_expected": total_expected,
                }
    ...
    if progress_callback:
        init_front = fast_non_dominated_sort(population)[0]
        progress_callback(
            {
                "event": "initial_population_done",
                "message": "Inicijalna populacija evaluirana.",
                "generation": 0,
                "evaluated": evaluated,
                "total_expected": total_expected,
                **_front_summary(init_front),
            }
        )
    ...
    for gen in range(G):
        while should_pause and should_pause():
            if progress_callback:
                progress_callback(
                    {
                        "event": "paused",
                        "message": f"Algoritam je pauziran prije generacije {gen + 1}.",
                        "generation": gen,
                        "evaluated": evaluated,
                        "total_expected": total_expected,
                    }
            ...
            time.sleep(0.5)
        if should_stop and should_stop():
            LOGGER.info("NSGA-II zaustavljen prije generacije %s.", gen + 1)
            break
            ...
            progress_callback(
                {
                    ...
                    "message": f"Pocinje generacija {gen + 1}/{G}",
                    "generation": gen + 1,
                    "evaluated": evaluated,
                    "total_expected": total_expected,
                }
        ...
        while len(Q) < N:
            while should_pause and should_pause():
                if progress_callback:
                    progress_callback(
                        {
                            "event": "paused",
                            "message": f"Algoritam je pauziran u generaciji {gen + 1}.",
                            "generation": gen + 1,
                            "evaluated": evaluated,
                            "total_expected": total_expected,
                        }
                ...
                time.sleep(0.5)
            if should_stop and should_stop():
                LOGGER.info("NSGA-II zaustavljen tokom generacije %s.", gen + 1)
                break
            ...
            if progress_callback:
                ...
                progress_callback(
                    {
                        "event": "candidate_start",
                        "message": f"Generacija {gen + 1}: trenira se kandidat {len(Q) + 1}/{N}",
                        "generation": gen + 1,
                        "evaluated": evaluated,
                        "total_expected": total_expected,
                        "candidate_phase": "offspring",
                        "candidate_index": len(Q) + 1,
                        "candidate_total": N,
                        "candidate_config": cfg,
                    }
                )
            ...
            if progress_callback:
                progress_callback(
                    {
                        "event": "child_eval",
                        "message": f"Generacija {gen + 1}: evaluirano potomaka {len(Q)}/{N}",
                        "generation": gen + 1,
                        "evaluated": evaluated,
                        "total_expected": total_expected,
                    }
        ...
        if progress_callback:
            current_front = fast_non_dominated_sort(population)[0]
            progress_callback(
                {
                    "event": "generation_done",
                    "message": f"Generacija {gen + 1} zavrsena.",
                    "generation": gen + 1,
                    "evaluated": evaluated,
                    "total_expected": total_expected,
                    **_front_summary(current_front),
                }
            )
    ...
    if progress_callback:
        progress_callback(
            {
                "event": "finished",
                "message": "NSGA-II izvrsavanje zavrseno.",
                "generation": min(G, len(final_fronts) - 1 if final_fronts else 0),
                "evaluated": evaluated,
                "total_expected": total_expected,
                **_front_summary(final_fronts[0]),
            }
        )
